...
let rec eval (e: expr) (env: environment) : value =
  ...
  |Mod (e1,e2)->(match eval e1 env,eval e2 env with
                ...
                |_,_->raise (Failure "incompatible types, Mod")
                )
  | Var n -> lookup n env
  ...
  | While (cond, body) ->
    ...
    | IfThenElse (cond,body1,body2)->
      (match eval cond stt with
        ...
        |Int _ -> raise (Failure "Type error on IfThenElse")
      )
    |For(str,e1,e2,body)->(let vals=lookup str ((str,eval e1 stt)::stt)
      in
      match eval (Eq(e2,Val vals)) ((str,eval e1 stt)::stt) with
      |Bool false->
        let e3=Add(Val (eval e1 stt),Val (Int 1))
        in
        let e4=Val (eval e2 stt)
        in
        exec (For(str,e3,e4,body)) (exec body ((str,eval e1 stt)::stt))
      |Bool true->exec body ((str,eval e1 stt)::stt)
      |_->raise(Failure("Type interpreter on For"))
    )
