...
let () =
  ...
  print_endline
  ("Intersection is associative so this gives the same result : " ^
                    Rational_interval.to_string (
  ...
  assert (Rational_interval.to_string
    (Rational_interval.create (19, 7) (22, 7)) =
            "(19/7, 22/7)");
  assert (Rational_interval.to_string
    (Rational_interval.create (15, 6) (8, 2)) =
            "(5/2, 4/1)");
  assert (Rational_interval.contains
    (Rational_interval.create (2, 3) (15, 7)) (6, 5));
  assert (not (Rational_interval.contains
    (Rational_interval.create (2, 3) (15, 7)) (2, 8)));
  assert (Rational_interval.intersect
    (Rational_interval.create ((-2), 1) (1, 1))
                        (Rational_interval.create ((-5), 3) (3, 2)) =
            Rational_interval.create ((-5), 3) (1, 1));
  assert (Rational_interval.to_string
            (Rational_interval.intersect
              (Rational_interval.create ((-5), 2) (6, 10))
                    (Rational_interval.create (5, 7) ((-3), 5))) =
            "Empty");
  print_endline("test cases all passesed!");
  ()
